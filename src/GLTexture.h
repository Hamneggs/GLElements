...
public:
	...
	*/
	...
	/*
	Loads a texture from an image located at <filePath>. This function
	is really inspired by mbsoftworks.sk's tutorial as well as the 
	FreeImage texture manager. I just wanted to do it myself for 
	understanding.

	Parameters:
		char * filePath:	The relative file path of the image.
		bool genMipMaps:	Whether or not to generate mipmaps on
							this texture.
		GLenum format:		The format of the texture. Use Google if you aren't
							using GL_RGB or GL_RGBA.
	...
	*/
	bool loadTextureImage(char * filePath, bool genMipMaps, GLenum format);
